...
st.title("📊 AS TAT 통합 관리 시스템 (리포트 정밀 추출)")
...
# --- 3. 메인 기능 탭 ---
tab0, tab1, tab2, tab3 = st.tabs(["🗂️ 마스터 관리", "📥 전량 입고 & 검증", "📤 고속 출고 매칭", "📈 리포트"])

# [TAB 0] 마스터 관리
with tab0:
    st.subheader("📋 마스터 정보 등록")
    m_file = st.file_uploader("마스터 파일 업로드", type=['csv', 'xlsx'], key="m_v5")
        ...
        if "master_lookup" not in st.session_state:
            ...
            for _, row in clean_df.iterrows():
                ...
                recs.append({
                    ...
                    "공급업체명": m_info.get("업체", "미등록"), "분류구분": m_info.get("분류", "미등록"),
                    "대상여부": m_info.get("AS구분", "미등록"), "입고일": str(to_pure_date(row.iloc[date_idx])), "상태": "출고 대기"
                })
...
with tab2:
    ...
    o_file = st.file_uploader("출고 CSV", type=['csv'], key="out_v5")
...
# [TAB 3] 리포트 (중복 제거 강화 로직)
with tab3:
    st.subheader("📈 TAT 리포트 (중복 제거 추출)")
    if st.button("📊 엑셀 리포트 생성 및 다운로드", use_container_width=True):
        all_d, offset = [], 0
        prog_msg = st.empty()
        
        # 1. DB에서 전체 데이터 안전하게 로드
        while True:
            # id 순으로 정렬하여 페이지네이션 시 중복 발생 방지
            res = supabase.table("as_history").select("*").order("id").range(offset, offset+999).execute()
            if not res.data: break
            all_d.extend(res.data)
            offset += 1000
            prog_msg.text(f"🔄 DB 데이터 읽는 중... {len(all_d):,}건 로드됨")
        
        if all_d:
            df = pd.DataFrame(all_d)
            
            # 2. 추출 후 메모리에서 압축코드 기준 최종 중복 제거 (보험)
            original_len = len(df)
            df = df.drop_duplicates(subset=['압축코드'], keep='last')
            final_len = len(df)
            
            if original_len != final_len:
                st.warning(f"⚠️ 리포트 생성 과정에서 중복 데이터 {original_len - final_len}건을 정제했습니다.")
            
            st.write(f"✅ 최종 추출 수량: **{final_len:,}** 건")
            st.dataframe(df.head(100)) # 미리보기
            ...
            st.download_button(
                ...
                file_name=f"AS_TAT_Final_Report_{time.strftime('%Y%m%d')}.xlsx",
