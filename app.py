...
# [데이터 정제 함수] 모든 공백 제거, 소수점 제거, 대문자화
def sanitize_code(val):
    if pd.isna(val) or str(val).strip() == "": return ""
    return str(val).split('.')[0].replace(" ", "").strip().upper()

# [날짜 정제 함수] 순수 date 객체 변환 (비교 연산용)
        ...
        if "master_lookup" not in st.session_state: st.error("⚠️ 마스터를 먼저 로드하세요.")
            ...
            try:
                ...
                while True:
                    ...
                    offset += len(res.data)
                    ui_msg.info(f"🔍 중복 체크 수집 중... ({offset:,})")
        ...
        try:
            ...
            if len(as_out) == 0: st.error("❌ 'AS 카톤 박스' 행 없음.")
                ...
                for i, (_, row) in enumerate(as_out.iterrows()):
                    ...
                    if code in db_lookup and db_lookup[code]:
                        for idx, db_r in enumerate(db_lookup[code]):
                            if to_pure_date(db_r['입고일']) <= out_date_obj:
                ...
                if upd_list:
                    ...
                    ui_msg.success(f"✅ {len(upd_list):,}건 반영 완료")
                else: st.error("❌ 일치 항목 없음")
        except Exception as e: st.error(f"오류: {e}")

# [TAB 3] 리포트 생성 (필터링 및 양식 복구)
with tab3:
    st.subheader("📈 분석 리포트 생성")
    
    def fetch_all():
        data, offset, m = [], 0, st.empty()
        while True:
            res = supabase.table("as_history").select("*").range(offset, offset+999).execute()
            if not res.data: break
            data.extend(res.data); offset += len(res.data)
            m.info(f"📥 데이터 수집 중... ({offset:,}건)")
    ...
    def prepare_excel(df):
        ...
        if '규격' not in df.columns: df['규격'] = "-"
        # [중요] 요청하신 컬럼 순서 고정
        target_cols = ['입고일', '자재번호', '자재명', '규격', '공급업체명', '분류구분', '출고일', 'TAT', '상태']
        df_final = df[[c for c in target_cols if c in df.columns]]
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='xlsxwriter') as wr:
            df_final.to_excel(wr, index=False, sheet_name='Report')
        return output.getvalue()

    c1, c2, c3 = st.columns(3)
    if c1.button("📊 1. 전체 리포트", use_container_width=True):
        st.session_state.r1 = prepare_excel(fetch_all())
    if c2.button("✅ 2. TAT 매칭건", use_container_width=True):
        df = fetch_all()
        st.session_state.r2 = prepare_excel(df[df['상태'] == '출고 완료'])
    if c3.button("⚠️ 3. 미등록/재입고", use_container_width=True):
        df = fetch_all()
        st.session_state.r3 = prepare_excel(df[df['상태'] != '출고 완료'])

    st.divider()
    d1, d2, d3 = st.columns(3)
    if "r1" in st.session_state: d1.download_button("📥 전체 다운로드", st.session_state.r1, "전체_리포트.xlsx", use_container_width=True)
    if "r2" in st.session_state: d2.download_button("📥 매칭건 다운로드", st.session_state.r2, "TAT_매칭_리포트.xlsx", use_container_width=True)
    if "r3" in st.session_state: d3.download_button("📥 미등록 다운로드", st.session_state.r3, "미등록_리포트.xlsx", use_container_width=True)
