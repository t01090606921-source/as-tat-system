...
def simple_clean(val):
    """자재번호를 문자열로 표준화 (가장 보수적이고 확실한 매칭)"""
...
st.title("⏱️ AS TAT 분석 시스템 (Full Version)")
    ...
    if master_file and st.button("🚀 마스터 DB 갱신", use_container_width=True):
        try:
            ...
            for _, row in m_df.iterrows():
                mat_no = simple_clean(row.iloc[0]) # A열
    ...
    if st.button("🔄 미등록 정보 재매칭", use_container_width=True):
        with st.spinner("재매칭 중..."):
            ...
            for row in h_res.data:
                m_info = m_lookup.get(simple_clean(row['자재번호']))
                if m_info:
...
with tab1:
    ...
    if in_file and st.button("입고 처리 실행"):
        ...
        for _, row in as_in.iterrows():
            mat_no = simple_clean(row.iloc[3])
            m_info = m_lookup.get(mat_no)
            new_recs.append({
                "압축코드": str(row.iloc[7]).strip(), "자재번호": mat_no,
...
try:
    ...
    if not all_data.empty:
        ...
        v_list = sorted(all_data['공급업체명'].unique())
        v_filter = f_col1.multiselect("🏢 공급업체", v_list)
        g_list = sorted(all_data['분류구분'].unique())
        g_filter = f_col2.multiselect("📂 분류구분", g_list)
        ...
        st.download_button(label="📥 현재 데이터 엑셀 다운로드", data=output.getvalue(), file_name=f"AS_Report_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.ms-excel")
