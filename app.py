...
st.title("⏱️ AS TAT 분석 시스템 (긴급 복구 완료)")

# --- 2. 사이드바: 관리 기능 ---
with st.sidebar:
    st.header("⚙️ 시스템 관리")
    
    # 마스터 건수 확인 (오류 방지를 위해 get 사용)
    try:
        m_count_res = supabase.table("master_data").select("자재번호", count="exact").execute()
        m_count = m_count_res.count if m_count_res.count is not None else 0
        st.info(f"📊 마스터 DB 등록: {m_count} 건")
    except:
        st.warning("마스터 정보를 불러올 수 없습니다.")
    ...
    if master_file and st.button("🚀 마스터 갱신", use_container_width=True):
        try:
            ...
            st.error(f"마스터 갱신 중 오류: {e}")

    st.divider()
    st.subheader("2. 데이터 보정")
    if st.button("🔥 정밀 재매칭 실행", use_container_width=True):
...
try:
    ...
    if res.data:
        ...
        v_list = sorted(data['공급업체명'].unique()) if '공급업체명' in data.columns else []
        s_list = sorted(data['상태'].unique()) if '상태' in data.columns else []
        
        v_f = c1.multiselect("🏢 공급업체 필터", v_list)
        s_f = c2.multiselect("🚚 상태 필터", s_list)
        ...
        if 'tat' in dff.columns:
            avg_tat = round(pd.to_numeric(dff['tat']).mean(), 1)
            m3.metric("평균 TAT", f"{avg_tat} 일")
        ...
        st.info("표시할 AS 데이터가 없습니다. 입고 엑셀을 업로드해 주세요.")
except Exception as e:
    st.error(f"리포트 표시 오류: {e}")
