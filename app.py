...
try:
    ...
    st.error("❌ Supabase 연결 실패. 프로젝트가 활성 상태인지 확인하세요.")
    st.stop()

st.title("📊 AS TAT 통합 관리 시스템 (전체 누적 데이터 모드)")
...
with st.sidebar:
    ...
    if not st.session_state.delete_mode:
        ...
        st.error("⚠️ 경고: 모든 데이터가 영구 삭제됩니다!")
        c1, c2 = st.columns(2)
        with c1:
...
with tab0:
    ...
    m_file = st.file_uploader("마스터 파일 업로드", type=['xlsx', 'csv'], key="master_upload")
...
# [TAB 2] 출고 처리
with tab2:
    st.subheader("📤 AS 출고 처리 (전체 누적 데이터 대조)")
...
# [TAB 3] 리포트 생성
with tab3:
    st.subheader("📈 AS TAT 분석 리포트 (전체 12만 건 조회)")
    if st.button("📊 리포트 생성"):
        with st.spinner("전체 누적 데이터를 불러오는 중..."):
            res = supabase.table("as_history").select("*").order("입고일", desc=True).limit(120000).execute()
            df = pd.DataFrame(res.data)
        
        if not df.empty:
            with st.spinner("데이터 가공 및 TAT 계산 중..."):
                ...
                st.write(f"📢 전체 데이터 개수: **{len(df):,}** 건")
                # 최신 데이터 5,000건만 화면 표시 (브라우저 보호)
                st.dataframe(df[cols].head(5000), use_container_width=True)
                if len(df) > 5000:
                    st.warning("⚠️ 데이터가 많아 상위 5,000건만 화면에 표시됩니다. 전체 10만 건 이상의 데이터는 아래 엑셀 다운로드를 이용하세요.")
                
                output = io.BytesIO()
                with pd.ExcelWriter(output, engine='xlsxwriter') as wr:
                    df[cols].to_excel(wr, index=False, sheet_name='TOTAL_TAT_REPORT')
                st.download_button("📥 10만건+ 전체 리포트 엑셀 다운로드", output.getvalue(), "AS_TAT_TOTAL_REPORT.xlsx")
        else:
