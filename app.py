...
st.title("📊 AS TAT 통합 관리 시스템 (대용량 최적화)")
...
with st.sidebar:
    ...
    # [수정] 대용량 데이터 타임아웃 방지용 분할 삭제 로직
    if st.button("⚠️ DB 데이터 전량 삭제", type="primary", use_container_width=True):
        st.session_state.clear()
        status_box = st.empty()
        try:
            with st.spinner("데이터를 분할 삭제 중입니다..."):
                while True:
                    # 한 번에 5,000건씩만 삭제 시도 (타임아웃 방지)
                    # 가장 작은 ID부터 5,000개를 찾아 삭제
                    ...
                    status_box.info(f"🗑️ 잔여 데이터를 지우는 중... (현재 처리 ID: {ids_to_del[-1]})")
                    time.sleep(0.1) # DB 부하 조절
