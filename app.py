...
def final_match_clean(val):
    """품목코드 매칭을 위한 최종 정제: 공백/줄바꿈 완벽 제거"""
    if pd.isna(val): return ""
    # 문자열로 변환 후 모든 종류의 공백(줄바꿈 포함) 제거
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("2. 정보 보정 (강력 대조)")
    if st.button("🔄 미등록 정보 정밀 재매칭", use_container_width=True):
        with st.spinner("마스터와 1:1 정밀 대조 중..."):
...
try:
    ...
    if not all_data.empty:
        ...
        st.download_button("📥 리포트 엑셀 다운로드", output.getvalue(), f"AS_Report.xlsx")
