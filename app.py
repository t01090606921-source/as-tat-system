...
st.title("📊 AS TAT 통합 관리 시스템 (전체 데이터 출력 모드)")
...
with tab0:
    ...
    m_file = st.file_uploader("마스터 파일 업로드", type=['xlsx', 'csv'])
...
with tab3:
    ...
    if st.button("📊 리포트 생성"):
        with st.spinner("데이터를 가져오는 중... (Max Rows 설정이 필요합니다)"):
            # .limit(120000)가 정상 작동하려면 서버 설정 변경이 선행되어야 함
