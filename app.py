...
# [정밀 정제] 모든 공백 제거, 소수점 제거, 대문자화
def sanitize_code(val):
    if pd.isna(val) or str(val).strip() == "": return ""
    return str(val).split('.')[0].replace(" ", "").strip().upper()

# [날짜 정제] 어떤 형식이든 순수 date 객체로 변환
...
# [TAB 0] 마스터 관리
with tab0:
    m_file = st.file_uploader("마스터 파일 선택", type=['xlsx', 'csv'], key="m_v_v")
    if m_file and st.button("🔄 마스터 로드", use_container_width=True):
        try:
...
# [TAB 1] 입고 처리
with tab1:
    i_file = st.file_uploader("입고 CSV 업로드", type=['csv'], key="i_v_v")
        ...
        if "master_lookup" not in st.session_state: st.error("⚠️ 마스터 먼저 로드!")
            ...
            try:
                ...
                for i, (_, row) in enumerate(as_in.iterrows()):
                    ...
                    if not in_date_obj: continue
                    code = sanitize_code(row.iloc[7])
                    
                    if f"{in_date_obj}|{code}" in existing:
                        dup_cnt += 1; continue
                ...
                ui_msg.success(f"✅ 입고 완료 (신규: {len(as_in)-dup_cnt:,}건)")
                ui_prog.progress(1.0)
            except Exception as e: st.error(f"오류: {e}")

# [TAB 2] 출고 처리 (날짜 검증 로직 정밀화)
with tab2:
    st.subheader("📤 AS 출고 반영")
    o_file = st.file_uploader("출고 엑셀 업로드", type=['xlsx'], key="o_v_v")
    if o_file and st.button("🚀 출고 반영 시작"):
        ...
        try:
            ...
            if len(as_out) == 0:
                st.error("❌ 'AS 카톤 박스' 데이터를 찾지 못했습니다.")
            else:
                ...
                for i, (_, row) in enumerate(as_out.iterrows()):
                    ...
                    out_date_obj = to_pure_date(row.iloc[6]) # 출고일 객체화
                    
                    if not out_date_obj: continue

                    if code in db_lookup:
                        for db_r in db_lookup[code]:
                            in_date_obj = to_pure_date(db_r['입고일']) # 입고일 객체화
                            # [핵심] 순수 date 객체끼리 비교하여 선후관계 검증
                ...
                if upd_list:
                    ...
                    st.error("❌ 일치하는 데이터가 없습니다. (압축코드 불일치 또는 입고일이 출고일보다 미래인 경우)")
                    # 디버깅: 왜 안되는지 샘플 출력
                    st.write("파일 코드 샘플:", [sanitize_code(x) for x in as_out.iloc[:3, 10]])
                    st.write("파일 날짜 샘플:", [to_pure_date(x) for x in as_out.iloc[:3, 6]])
        except Exception as e: st.error(f"오류: {e}")
...
with tab3:
    if st.button("📊 리포트 생성"):
        ...
        try:
            ...
            st.session_state.report = out.getvalue(); ui_msg.success("✅ 완료")
        except Exception as e: st.error(f"오류: {e}")
