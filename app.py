            ...
            with st.status("📥 입고 데이터 처리 중...", expanded=True) as status:
                try:
                    ...
                    while True:
                        ...
                        offset += len(res.data)
                        status.write(f"🔍 기존 데이터 로드 중... ({offset:,} / {total_db:,})")
                    ...
                    for i, (_, row) in enumerate(as_in.iterrows()):
                        ...
                        if len(recs) >= 200:
                            ...
                            recs = []
                            status.write(f"🚀 저장 중... ({i+1:,} / {total_in:,})")
                    if recs: supabase.table("as_history").insert(recs).execute()
                    status.update(label="✅ 입고 완료", state="complete")
                    st.success(f"신규 입고: {total_in-dup_cnt:,}건 / 중복 제외: {dup_cnt:,}건")
                except Exception as e: st.error(f"오류: {e}")

# --- [TAB 2] 출고 처리 (선후 관계 검증) ---
with tab2:
    out_file = st.file_uploader("출고 결과 엑셀", type=['xlsx'], key="out_up")
    if out_file and st.button("🚀 검증 후 출고 반영"):
        with st.status("📤 출고 유효성 검사 및 업데이트 중...", expanded=True) as status:
            try:
                df_out = pd.read_excel(out_file).fillna("")
                as_out = df_out[df_out.iloc[:, 3].astype(str).str.contains('AS 카톤 박스', na=False)].copy()
                
                # DB 전체 로드 (조인 대신 메모리 대조가 대용량에선 더 빠름)
                ...
                for i, (_, row) in enumerate(as_out.iterrows()):
                    ...
                    for db_row in matched:
                        ...
                        if str(db_row['입고일']) <= ex_out_date:
                            ...
                            found = True
                    if not found: skip_cnt += 1
                
                if update_list:
                    for idx, item in enumerate(update_list):
                        supabase.table("as_history").update({"출고일": item['출고일'], "상태": "출고 완료"}).eq("id", item['id']).execute()
                        if idx % 100 == 0: status.write(f"⏳ 업데이트 중... ({idx:,} / {len(update_list):,})")
                    status.update(label="✅ 출고 반영 완료", state="complete")
                    st.success(f"반영: {len(update_list):,}건 / 제외: {skip_cnt:,}건")
            except Exception as e: st.error(f"오류: {e}")

# --- [TAB 3] 분석 리포트 (순서 고정 추출) ---
with tab3:
    if st.button("📊 리포트 생성 (12만 건 무제한 추출)", use_container_width=True):
        with st.status("📈 데이터를 수집하여 엑셀을 생성 중입니다...", expanded=True) as status:
            all_data, offset = [], 0
            while True:
                ...
                offset += len(res.data)
                status.write(f"📥 데이터 수집 중... ({offset:,} 건)")
