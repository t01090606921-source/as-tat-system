...
with st.sidebar:
    ...
    # [검증 1] 실시간 카운트: DB에 데이터가 실제로 들어가는지 확인하는 용도
    ...
    if st.button("⚠️ DB 데이터 전량 삭제", type="primary", use_container_width=True):
        ...
        with st.spinner("데이터를 비우는 중입니다..."):
            try:
                ...
                st.info("💡 계속 오류가 발생하면 Supabase RLS 정책에서 'DELETE'가 허용되었는지 확인하세요.")
...
with tab0:
    ...
    st.info("입고 시 업체명과 분류를 매칭하기 위한 마스터 파일을 업로드하세요.")
    ...
    if m_file_mgmt and st.button("🔄 마스터 데이터 로드", use_container_width=True):
        with st.spinner("데이터 로드 중..."):
            try:
                ...
                st.success(f"✅ {len(new_lookup):,}건의 기준 정보가 메모리에 로드되었습니다.")
            except Exception as e:
                st.error(f"❌ 마스터 로드 실패: {e}")

# --- [TAB 1] 입고 처리 (12만 건 대응 안전 로직) ---
with tab1:
    st.info("💡 대용량 입고 시 서버 과부하를 막기 위해 분할 전송 모드가 활성화됩니다.")
    ...
    with col1:
        if "master_lookup" in st.session_state:
            ...
            use_existing = False
            st.warning("⚠️ [마스터 관리] 탭을 먼저 완료하세요.")
        ...
        try:
            ...
            if not lookup:
                ...
                if i_df is not None:
                    ...
                    if total > 0:
                        recs = []
                        batch_size = 100 # [검증 4] 502 에러 방지를 위한 묶음 크기 최적화
                        ...
                        p_bar.progress(1.0)
                        status_text.success(f"🎊 {total:,}건이 성공적으로 저장되었습니다.")
                    else:
                        st.error("❌ 'A/S 철거' 대상 데이터가 없습니다.")
        except Exception as e:
            st.error(f"❌ 입고 중 오류: {e}")

# --- [TAB 2] 출고 처리 ---
with tab2:
    st.info("📤 출고 엑셀을 업로드하여 상태를 업데이트합니다.")
        ...
        try:
            with st.spinner("업데이트 중..."):
                ...
                if not as_out.empty:
                    ...
                    st.success(f"✅ 총 {count:,}건의 출고 정보 반영 완료.")
                else:
                    st.warning("⚠️ 업데이트할 데이터가 없습니다.")
        except Exception as e:
            st.error(f"❌ 출고 처리 오류: {e}")

# --- [TAB 3] 분석 리포트 (무제한 추출 및 컬럼 순서 고정) ---
with tab3:
    st.subheader("📈 대용량 데이터 통합 분석")
    
    if st.button("📊 전체 리포트 생성 (12만 건 무제한 수집)", use_container_width=True):
        with st.spinner("🚀 DB에서 조각난 데이터를 모으고 있습니다... (약 1분 소요)"):
            try:
                all_data = []
                offset = 0
                fetch_size = 1000
                ...
                # [검증 5] Pagination 루프: 데이터가 없을 때까지 1,000건씩 반복 호출
                while True:
                ...
                if not all_data:
                    ...
                    def make_bin(target_df):
                        ...
                        # [검증 6] 요청하신 컬럼 순서 정확히 배치
                        target_cols = [
                            '입고일', '자재번호', '자재명', '규격', 
                            '공급업체명', '분류구분', '압축코드', '출고일', 'tat'
                        ]
                        # 실제 존재하는 컬럼만 필터링 (에러 방지용)
    ...
    if st.session_state.get("data_ready"):
        st.divider()
        st.write(f"📂 **분석 결과 다운로드 (대상: {st.session_state.total_count:,}건)**")
        c1, c2, c3 = st.columns(3)
        with c1: 
            if st.session_state.bin_tat: st.download_button("📥 1. 출고완료 리포트", st.session_state.bin_tat, "1_tat_done.xlsx", use_container_width=True)
        ...
        with c3: 
            st.download_button("📥 3. 전체 합계 데이터", st.session_state.bin_total, "3_all_history.xlsx", use_container_width=True)
