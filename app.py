...
try:
    ...
    st.error("⚠️ Supabase 접속 설정을 확인해주세요. (secrets.toml 설정 필요)")
    st.stop()
...
with st.sidebar:
    ...
    if not st.session_state.delete_mode:
        ...
        st.warning("⚠️ 정말로 모든 데이터를 삭제하시겠습니까?")
        c1, c2 = st.columns(2)
        with c1:
    ...
    if m_file and st.button("🔄 마스터 데이터 로드"):
        ...
        if m_df is not None:
            try:
                ...
                st.success(f"✅ 마스터 데이터 {len(st.session_state.master_lookup)}건 로드 완료")
            except Exception as e:
                st.error(f"데이터 매싱 중 오류 발생: {e}")
        else:
            st.error("파일을 읽을 수 없습니다. 인코딩이나 파일 형식을 확인하세요.")
...
with tab1:
    ...
    if i_file and st.button("🚀 입고 데이터 반영"):
        if "master_lookup" not in st.session_state:
            st.warning("⚠️ 마스터 데이터를 먼저 로드해야 입고 처리가 가능합니다.")
        else:
            ...
            for _, row in as_out.iterrows():
                ...
                if dest == "주식회사디지타스":
                    ...
                    if target_id:
                        ...
                        db_data[target_id].update(upd) # 메모리 갱신
                        success_count += 1
                
                # 2. 벤더 출고
                else:
                    # 1순위: 디지타스 거친 행 / 2순위: 바로 나가는 행
                    for rid, rdata in db_data.items():
                        if sanitize_code(rdata['압축코드']) == code and not rdata.get('벤더_출고일'):
                            # 디지타스 출고일 유무와 상관없이 매칭 시도
                            target_id = rid; break
                    
                    if target_id:
                        upd = {"벤더_출고지": dest, "벤더_출고일": str(out_date), "상태": "벤더 출고 완료"}
                        supabase.table("as_history").update(upd).eq("id", target_id).execute()
                        del db_data[target_id] # 완료 데이터 제거
                        success_count += 1
...
with tab3:
    ...
    if st.button("📊 리포트 생성 및 새로고침"):
        ...
        if not df.empty:
            ...
            # 최종 출고일 기준으로 TAT 계산 (벤더 우선, 없으면 디지타스)
