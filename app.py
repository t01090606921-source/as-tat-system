...
def super_clean(val):
    """자재번호를 극한까지 클리닝 (앞의 0 제거, 특수문자 제거, 대문자화)"""
    if pd.isna(val): return ""
    # 1. 문자열화 및 공백 제거
    s = str(val).strip().upper()
    # 2. 소수점 .0 제거
    if s.endswith('.0'): s = s[:-2]
    # 3. 숫자와 영문 제외한 모든 것 제거
    s = re.sub(r'[^A-Z0-9]', '', s)
    # 4. 앞자리에 붙은 의미 없는 '0'들 제거 (예: 000123 -> 123)
    s = s.lstrip('0')
    return s

st.set_page_config(page_title="AS TAT 시스템", layout="wide")
st.title("⏱️ AS TAT 분석 시스템 (v2.1)")
        ...
        with st.spinner("유연한 매칭 알고리즘 가동 중..."):
            ...
            for row in h_res.data:
                ...
                if m_info:
                    ...
                    up_cnt += 1
            st.success(f"✅ {up_cnt}건의 정보가 성공적으로 보정되었습니다!")
            st.rerun()
    ...
    if in_file and st.button("입고 처리 실행"):
        with st.spinner("마스터 대조 중..."):
            ...
            for _, row in as_in.iterrows():
                mat_no = super_clean(row.iloc[3]) # D열
...
try:
    ...
    if not all_data.empty:
        ...
        fin = all_data[all_data['상태'] == '출고 완료']
        m2.metric("평균 TAT", f"{round(pd.to_numeric(fin['tat']).mean(), 1) if not fin.empty else 0} 일")
        m3.metric("미등록 건수", f"{len(all_data[all_data['공급업체명'] == '미등록'])} 건")
